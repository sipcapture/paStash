...
function InputAmqp() {
  ...
  this.mergeConfig({
    ...
    optional_params: ['topic', 'durable', 'retry_delay', 'heartbeat', 'type', 'username', 'password', 'vhost', 'prefetch', 'queue_name'],
    default_values: {
      'durable': true,
      'retry_delay': 3000,
      'heartbeat': 10,
  ...
  this.connected_callback = function(channel) {
    channel.assertExchange(this.exchange_name, this.topic ? 'topic' : 'fanout', {durable: this.durable}, function(err) {
      if (err) {
        ...
        let queue_options = {};
        queue_options.exclusive = !prefetch;
        queue_options.durable   = !!prefetch;
        channel.assertQueue(this.queue_name, queue_options, function(err, queue) {
          if (err) {
            logger.error('Unable to assert locale queue', err);
          } else {
            if (prefetch) {
              process.on('USRSIGCONSUME', (m) => {
                channel.ack(m);
              });
            }
