...
if (io) {
  metrics = {
	amqp_in_rps: io.meter({
	  name: 'in req/sec',
	  type: 'meter',
	}),
	amqp_in_err_rps: io.meter({
	  name: 'err req/sec',
	  type: 'meter',
	}),
	amqp_in_err_driver: io.meter({
...
InputAmqp.prototype.start = function(callback) {
  ...
  this.connected_callback = function(channel) {
	...
	if (metrics) metrics.amqp_in_err_driver.mark();
      }
      else {
		...
		if (metrics) metrics.amqp_in_err_driver.mark();
		    ...
		    if (metrics) metrics.amqp_in_rps.mark();
                    this.emit('data', parsed);
                  }.bind(this), function(data) {
		    if (metrics) metrics.amqp_in_err_rps.mark();
