...
function InputVoicenter() {
    ...
    this.mergeConfig({
        name: 'Voicenter',
        optional_params: ['api', 'token', 'primaryReconnectTimeout'],
...
const convertUrl = (server) => {
    ...
    return protocol + server.split(":")[0];
}

InputVoicenter.prototype.loadServers = async function () {
    if (servers) return true;

    if (this.primaryReconnectTimeout) primaryReconnectTimeout = this.primaryReconnectTimeout;
