	...
	if (direction&&direction[1]) {
		switch (direction[1]) {
			case 'Sent':
					...
					if (ip_from[2]!=device)
					{
						ipcache.dstIp = ip_from[2];
					}
					else
					{
						ipcache.dstIp = ip_to[2];
					}
			...
			case 'Received':
					...
					if (ip_from[2]!=device)
					{
						ipcache.srcIp = ip_from[2];
					}
					else
					{
						ipcache.srcIp = ip_to[2];
					}
			break;
		}
	}
	
	date_regex = /([A-Za-z]{3}\s\d{2}\s\d{2}:\d{2}:\d{2}\.\d+)/;
	date=date_regex.exec(line_clean)[1];
	if (date&&date[1]) {
		ipcache.xdate = moment(date, 'MMM DD HH:mm:ss.SSS');
		ipcache.ts = ipcache.xdate.unix();
		ipcache.usec = ipcache.xdate.millisecond() * 1000;
	}
	payloadfinal_regex=/^[A-Za-z]{3}\s\d{2}\s\d{2}:\d{2}:\d{2}\.\d+.*?:\s.*SIP\/Msg\/ccsipDisplayMsg:\n(Sent|Received):\n/g;
