/*
   ...
   (C) 2017 QXIP BV
*/
const base_filter = require('../lib/base_filter'),
    util = require('util'),
    logger = require('log4node'),
    fs = require('fs'),
    asyncChainable = require('async-chainable'),
...
FilterCommand.prototype.start = function (callback) {
    ...
    for (var plugin in this.pluginsData) {
        logger.info('Start loading 2', plugin);
        try {
...
FilterCommand.prototype.process = function (data) {
    ...
    try {
        try {
            data = Object.assign(JSON.parse(data[this.field]), { _operationId: data._operationId || null });
        } catch (err) {
            data = Object.assign(data[this.field], { _operationId: data._operationId || null });
        }
        ...
        if (data[this.fieldCommandList]) {
            ...
            if (this.commandList.length === 0) {
                ...
                if (this.commandList.constructor.name === "String") {
                    commandNameList.push(this.commandList);
                } else {
        ...
        commandNameList.forEach(function (commandName, index) {
            ...
            const command = function (next) {
                ...
                let taskFinishHOF = function (logEnd, fn) {
                    return function () {
