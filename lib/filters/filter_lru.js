...
function FilterLRU() {
  ...
  this.mergeConfig({
    name: 'LRU',
    optional_params: ['field', 'value_field', 'target_field', 'action', 'shared'],
    default_values: {
      'target_field': 'cache',
      'shared': false
    },
...
FilterLRU.prototype.process = function(data) {
  if (this.action === 'set'){
	...
	if(!data[this.value_field]) return data;
	this.cache_do(data[this.field], data[this.value_field], function(err,result){
	      this.emit('output', data);
	}.bind(this));
  } else if (this.action === 'get'){
  	logger.debug('LRU get... ',data[this.field],data[this.target_field]);
