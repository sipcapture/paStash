...
FilterAppSonus.prototype.process = function(data) {
   ...
   if (line.indexOf('sending from') !== -1) {
	...
	//console.log('in',ipcache);
	   ...
	   var regex = /\d\d\d\d\s(.*)\s(.*):\d.*\: received msg for CallId:(.*) from IP\/port:(.*)\/(.*), Local IP\/port:(.*)\/(.*), SMM:(.*)/g;
	   var ip = regex.exec(line);
	   ipcache.srcIp = ip[4];
	   ...
	   var regex = /\d\d\d\d\s(.*)\s(.*):\d.*\: sent msg for CallId:(.*) to IP\/port:(.*)\/(.*), Local IP\/port:(.*)\/(.*), SMM:(.*)/g;
	   var ip = regex.exec(line);
	   ipcache.srcIp = ip[4];
	   ipcache.srcPort = ip[5];
	   ipcache.dstIp = ip[6];
   ...
   } else if (!line.startsWith("RAW PDU") ) {
	        hold = true;
