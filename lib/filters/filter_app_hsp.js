...
FilterAppHsp.prototype.start = function(callback) {
  ...
  if (this.pmx){
	...
	try {
		...
		this.counter_in = Probe.counter({
		  name : 'HSP Input'
		});
		this.counter_out = Probe.counter({
		  name : 'HSP Output'
		});
		this.counter_err = Probe.counter({
		  name : 'HSP Errors'
		});
		this.counter_queue = Probe.counter({
		  name : 'HSP Queue'
		});
...
FilterAppHsp.prototype.process = function(raw) {
	...
	if (!raw.message) {
		if (this.pmx) this.counter_err.inc();
		return;
	}

        if (this.pmx) { this.counter_queue.inc(); this.counter_in.inc(); }
	try {
		...
		if (this.omit) raw.message = omit(raw.message, this.omit);
	 	...
	 	if (this.pmx) { this.counter_queue.dec(); this.counter_out.inc(); }
		return raw.message;
	    }
		...
		if (this.sqlite_db && raw.message.duration > 0 && raw.message.ruri_user) {
			...
			console.log('ERROR:',e);
		 	if (this.pmx) { this.counter_queue.dec(); this.counter_out.inc(); this.counter_err.inc(); }
			return raw.message; }
