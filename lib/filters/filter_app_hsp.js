...
function FilterAppHsp() {
  ...
  this.mergeConfig({
    name: 'AppHsp',
    optional_params: ['correlation_hdr', 'sqlite_db','default_cc','strip_local','strip_dialprefix','strip_testuser','hepic_port','hepic_path', 'hepic_token','hepic_host', 'groupby', 'omit', 'splitter' ],
    default_values: {
...
FilterAppHsp.prototype.process = function(raw) {
	...
	try {
		...
		if (this.sqlite_db && raw.message.duration > 0 && raw.message.ruri_user) {
			...
			try {
			  if(raw.message.cstm1 && raw.message.cstm2){
				var cstmarr1 = raw.message.cstm1.split(';');
				var cstmarr2 = raw.message.cstm2.split(';');
				var cstmarr3 = raw.message.cstm3.split(';');
				var cstmarr4 = raw.message.cstm4.split(';');
				var cstmarr5 = raw.message.cstm5.split(';');
				var cstmcount = cstmarr1.length;
				for (i=0;i<cstmcount;i++){
					if (!cstmarr1[i]||cstmarr1[i] === '') return;
					var tmpmessage = raw.message;
					tmpmessage.cstm1 = cstmarr1[i];
					tmpmessage.cstm2 = cstmarr2[i];
					tmpmessage.cstm3 = cstmarr3[i];
					tmpmessage.cstm4 = cstmarr4[i];
					tmpmessage.cstm5 = cstmarr5[i];
					this.emit('output', tmpmessage);
				}
			  }
			} catch(e) { logger.error(e); this.emit('output',raw.message); }
