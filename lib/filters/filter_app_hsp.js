...
function FilterAppHsp() {
  ...
  this.mergeConfig({
    name: 'AppHsp',
    optional_params: ['correlation_hdr', 'sqlite_db','default_cc','default_regex'],
    default_values: {
...
FilterAppHsp.prototype.process = function(raw) {
	...
	try {
		...
		// CDR Timestamp
		raw.message['@timestamp'] = Date.now();
		...
		if (this.sqlite_db && raw.message.duration > 0) { 
			...
			bnumber = bnumber.replace(/^0([1-9])/, country + "\$1");
			bnumber = bnumber.replace(/^00/, "");
			bnumber = bnumber.replace(/^5000/, country);
