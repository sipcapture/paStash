...
function OutputOracle() {
  ...
  this.mergeConfig({
    name: 'Oracle',
    optional_params: ['username', 'password', 'connectString', 'table', 'debug', 'simulate', 'check_interval', 'schema', 'schemafile'],
    default_values: {
	debug: false,
	simulate: false
    },
...
OutputOracle.prototype.start = function(callback) {
  ...
  if(!this.schemafile && !this.schema){ logger.error('Missing Schema! Exiting...'); return; }
  if(!this.connectString||!this.username||!this.password||!this.table) { logger.error('Missing DB parameters! Exiting...'); return; }

  try {
	if (this.schemafile) this.dbschema = require(this.schema);
