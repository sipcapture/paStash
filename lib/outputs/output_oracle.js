...
function OutputOracle() {
  ...
  this.mergeConfig({
    name: 'Oracle',
    optional_params: ['username', 'password', 'connectString', 'table', 'debug', 'simulate', 'check_interval', 'schema', 'schema_file', 'insert_use', 'autoCommit'],
    default_values: {
	debug: false,
	insert_use: false,
	autoCommit: true,
...
OutputOracle.prototype.start = function(callback) {
  ...
  if(!this.simulate){
	    ...
	    if (this.autoCommit) oracledb.autoCommit = true;
  }
		...
		this.dbschema.forEach(function(row,i){
		  try {
			...
			values.push( ':' + row.field );
				...
				logger.debug('Default Insert! Missing Field in Source:',row.field);
			}
		  } catch(e) { logger.error(e); return; }
		});

		var qquery = "INSERT INTO "+this.table;
		    if (this.insert_use) qquery += " USE ("+cols.join(',')+")";
		    qquery += " VALUES ("+values.join(',')+")";
