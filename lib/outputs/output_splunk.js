...
function OutputSplunk() {
  ...
  this.mergeConfig({
    name: 'Splunk',
    optional_params: ['token','splunk_url','batchInterval','maxBatchCount','maxBatchSize', 'debug', 'index','sourcetype', 'source', 'host', 'timefield'],
    default_values: {
    	batchInterval: 1000,
    	maxBatchCount: 10,
    	maxBatchSize: 1024,
...
OutputSplunk.prototype.process = function(data) {
	...
	if (this.timefield) {
		if (data[this.timefield]) {
			this.metadata.time = data[this.timefield];	
		}
	}
