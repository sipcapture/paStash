...
function OutputKafka() {
  ...
  this.mergeConfig({
    name: 'Kafka',
    optional_params: ['kafkaHost', 'topic', 'partition', 'threshold_down', 'check_interval' ],
    default_values: {
	debug: false,
	topic: 'hepic',
	threshold_down: 10
...
OutputKafka.prototype.process = function(data) {
	...
	if (this.partition && data[this.partition]) d[0].partition = data[this.partition];
	if (this.debug) logger.info("Preparing to send to Kafka", d );
