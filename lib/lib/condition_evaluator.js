...
function force_number(x) {
  if (typeof x !== 'number') {
    ...
    if (isNaN(z)) {
      return false;
      // throw new Error('Unable to cast to int : ' + JSON.stringify(x), x);
    }
    else {
      x = z;
    }
...
compute = exports.compute = function(cond, data, op_override) {
  ...
  if (op === '==') {
    ...
    return get_bool(cond.left,data) && get_bool(cond.right,data);
  }
  else if (op === 'or') {
    return get_bool(cond.left,data) || get_bool(cond.right,data);
  }
  else if (op === 'xor') {
    var r4 = get_bool(cond.right,data);
    return get_bool(cond.left,data) ? ! r4 : r4;
  }
  else if (op === 'nand') {
    return ! compute(cond, data, 'and');
  }
  else if (op === 'miss') {
    return data[cond.left];
  }
  else {
